...
function school_header($code,$short=false){
	...
	if($short){
		...
		echo "<div style='margin-bottom:5px;'><a style='color:blue;text-decoration:none;border-bottom:1px dotted blue;' href='?code=$code' title='$name - $venue $addr'>$pc, $name @ $venue</a></div>\n";
		echo "<div style='margin-bottom:1px;font-size:0.8em;'>$addr</div>";
		echo "<div style='margin-bottom:10px;font-size:0.8em;'><a target='_blank' style='text-decoration:none;color:#444;border-bottom:1px dotted #444;' href='$eurl'>" . parse_url($eurl, PHP_URL_HOST) . "</a></div>";
	} else {
...
function school_courses($code,$only_level=false,$only_day=false){
	...
	if(!$courses)	return false;
	sort($courses);
	echo "<table cellpadding=0 cellspacing=0 border=0 style='margin-bottom:15px;width:480px;'>";
	foreach($courses as $num => $course){
		...
		echo "<td style='padding-right:5px;width:55px;'>" . $lg->get($day,"day") . "</td>";
		echo "<td style='padding-right:5px;width:55px;'>" . $hour . "</td>";
		echo "<td style='padding-right:5px;width:200px;padding-bottom:1px;'>" . $lg->get($level,"level") . (($comment)? $comment : "") . "</td>";
		echo "<td style='padding-right:5px'>" . $teachers . "</td>";
		echo "</tr>\n";
	}
	echo "</table>";
}
