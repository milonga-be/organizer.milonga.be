...
$default_path="http://agenda.milonga.be" . $default_path;
...
$uid_arr = array();
	...
	$dayofweek = localizeDate ("%a, %e %b %Y", strtotime($thisdate));
	if (isset($master_array[($thisdate)]) && sizeof($master_array[($thisdate)]) > 0) {
		$rss .= '<item>'."\n";
		$rss .= "<title>http://agenda.milonga.be : $dayofweek</title>\n";
		$rss .= "<link>$default_path/day.php?getdate=$thisdate</link>\n";
		$rss .= "<dc:creator>agenda@milonga.be</dc:creator>\n";
		$rss .= "<pubDate>$dayofweek 06:00:00 +0000</pubDate>\n";
		$rss .= "<guid isPermaLink=\"false\">$default_path/day.php?getdate=$thisdate</guid>\n";
		$rss .= "<description><![CDATA[\n<ul>\n";
		foreach ($master_array[("$thisdate")] as $event_times) {
			foreach ($event_times as $uid=>$val) {
				$event_text 	= stripslashes(urldecode($val["event_text"]));
				$event_text 	= strip_tags($event_text, '<b><i><u>');
				$rss_title		= htmlspecialchars ("$event_text");
				$rss .= "<p>&bull; $rss_title </p>\n";
				$events_count++;
			}
		}
		$rss .= "</ul>\n]]>\n</description>";
