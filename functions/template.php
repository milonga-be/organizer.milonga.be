...
class Page {
			...
			if ((($regs[1] == $parse_month) && ($printview == 'month')) || (($key == $getdate) && ($printview == 'day')) || ((($key >= $week_start) && ($key <= $week_end)) && ($printview == 'week')) || ((substr($regs[1],0,4) == $parse_year) && ($printview == 'year'))) {
				...
				foreach ($val as $new_val) {
					...
					if ($new_val2['event_text']) {	
						...
						$description 	= htmlize(stripslashes(urldecode($new_val2['description'])));
						...
						$ev_bullet="&bull;";
						//$event_text=strtoupper($event_text);
						$ev_text2=strtoupper($event_text);
						$ev_text2=trim(str_replace(Array("-",":",",","/")," ",$ev_text2));
						$words=explode(" ",$ev_text2);
						$ev_type=$words[0];
						//$ev_type=trim(substr($ev_text2,0,strpos($ev_text2," "))); // take first word
						switch ($ev_type){
						case "CONCERT":  $ev_bullet="<span style='color: #A00; font-size: 1.5em;'>&fnof;</span>"; break;
						case "SALES":  $ev_bullet="<span style='color: #F60; font-size: 1.5em;'>$</span>"; break;
						case "MILONGA":  $ev_bullet="<span style='color: #600; font-size: 1.5em;'>&hearts;</span>"; break;
						case "PRACTICA": $ev_bullet="<span style='color: #339; font-size: 1.5em;'>&diams;</span>"; break;
						case "SALON":    $ev_bullet="<span style='color: #A00; font-size: 1.5em;'>&hearts;</span>"; break;
						case "PERFORMANCE":  $ev_bullet="<span style='color: #A00; font-size: 1.5em;'>&pi;</span>"; break;
						case "WORKSHOP": $ev_bullet="<span style='color: #0A0; font-size: 1.5em;'>&spades;</span>"; break;
						case "EVENT": $ev_bullet="<span style='color: #0A0; font-size: 1.5em;'>&bull;</span>"; break;
						}
						$search		= array('{EVENT_START}', '{EVENT_BEGIN}', '{EVENT_TEXT}', '{DESCRIPTION}', '{LOCATION}', '{_EVENT_TEXT}','{EVENT_BULLET}','{DAYOFMONTH}');
						$replace	= array($event_start, $event_begin, $event_text, $description, $location, urlencode($event_text), $ev_bullet, $dayofmonth);
						$events_tmp = str_replace($search, $replace, $events_tmp);
						$some_events .= $events_tmp;
						$events_tmp	= $loop_event;
					}
				}
				$day_tmp  = str_replace('{FULLDATE}', $key, $day_tmp);
	...
	function draw_filter($template_p,$filterstring) {
		global $template, $getdate, $cal, $master_array, $daysofweek_lang, $week_start_day, $printview, $dateFormat_day, $timeFormat, $week_start, $week_end, $lang;
		preg_match("!<\!-- loop events on -->(.*)<\!-- loop events off -->!is", $this->page, $match1);
		preg_match("!<\!-- switch some_events on -->(.*)<\!-- loop events on -->!is", $this->page, $match3);
		$loop_event		= trim($match1[1]);
		...
		$filterstring_without_semicolons = str_replace([' :',':'], '', $filterstring );
		...
		foreach($master_array as $key => $val) {
			...
			if ((($regs[1] == $parse_month) && ($printview == 'month')) || (($key == $getdate) && ($printview == 'day')) || ((($key >= $week_start) && ($key <= $week_end)) && ($printview == 'week')) || ((substr($regs[1],0,4) == $parse_year) && ($printview == 'year'))) {
				...
				$dayofmonth = strtotime ($key);
				$dayofmonth = localizeDate ($dateFormat_day, $dayofmonth);
				$dayofmonth_short = date( 'l D M' , strtotime ($key));
				$events_tmp = $loop_event;
				$day_tmp	= $loop_day;
				...
				foreach ($val as $new_val) {
					...
					if ($new_val2['event_text']) {	
						...
						$ev_bullet="&bull;";
						$search		= array('{EVENT_START}', '{EVENT_TEXT}', '{DESCRIPTION}', '{LOCATION}', '{_EVENT_TEXT}', '{EVENT_BULLET}','{DAYOFMONTH}','{DAYOFMONTHSHORT}');
						$replace	= array($event_start, $event_text, $description, $location, urlencode($event_text),$ev_bullet,$dayofmonth.'*',$dayofmonth_short);
						$events_tmp = str_replace($search, $replace, $events_tmp);
						
						if(stristr($event_text,$filterstring) || stristr(str_replace(' :',':',$event_text), $filterstring) || strtolower(substr($event_text, 0, strlen($filterstring_without_semicolons))) === $filterstring_without_semicolons ){
							$some_events .= $events_tmp;
						}
						$events_tmp	= $loop_event;
					}
				}
				$day_tmp  = str_replace('{FULLDATE}', $getdate, $day_tmp);
				$day_tmp  = str_replace('{DAYOFMONTH}', $dayofmonth, $day_tmp);
				if(strlen($some_events)>0){
				$final   .= $day_tmp.$some_events;
				}
				unset ($day_tmp, $some_events);
			}
	...
	function draw_week($template_p) {
		...
		foreach ($day_array as $key) {
			...
			if (ereg("([0-9]{1,2}):00", $key)) {
				$weekdisplay .= '<tr>';
				$weekdisplay .= '<td colspan="4" rowspan="' . (60 / $gridLength) . '" align="center" valign="top" wwidth="60" class="timeborder">'.$key.'</td>';
				$weekdisplay .= '<td bbgcolor="#a1a5a9" width="1" height="' . $gridLength . '"></td>';
			} elseif ($cal_time == $day_start) {
				$size_tmp = 60 - (int)substr($cal_time,2,2);
				$weekdisplay .= '<tr>';
				$weekdisplay .= '<td colspan="4" rowspan="' . ($size_tmp / $gridLength) . '" align="center" valign="top" wwidth="60" class="timeborder">'.$key.'</td>';
				$weekdisplay .= '<td bbgcolor="#a1a5a9" width="1" height="' . $gridLength . '"></td>';
			} else {
				$weekdisplay .= '<tr>';
				$weekdisplay .= '<td bbgcolor="#a1a5a9" width="1" height="' . $gridLength . '"></td>';
			...
			for ($week_loop=0; $week_loop<$week_length; $week_loop++) {
				...
				if (sizeof($event_length[$thisday]) == 0) {
					...
					$weekdisplay .= '<td wwidth="' . $colspan_width . '" colspan="' . $nbrGridCols[$thisday] . '" ' . $class . '>&nbsp;</td>'."\n";
					...
					for ($i=0;$i<sizeof($event_length[$thisday]);$i++) {
						...
						switch ($event_length[$thisday][$i]["state"]) {
							case "begin":
								...
								$colspan_width = round((80 / $nbrGridCols[$thisday]) * $drawWidth);
								$weekdisplay .= '<td wwidth="'.$colspan_width.'" rowspan="' . $event_length[$thisday][$i]['length'] . '" colspan="' . $drawWidth . '" align="left" valign="top" class="eventbg2_'.$event_calno.'">'."\n";
	...
	function draw_month($template_p, $offset = '+0', $type) {
		...
		do {
				...
				if ($type != 'small') {
					foreach ($master_array[$daylink] as $cal_time => $event_times) {
						foreach ($event_times as $uid => $val) {
							...
							if (!isset($val['event_start'])) {
								if ($type == 'large') {
									$switch['ALLDAY'] .= '<div class="V10"><img src="templates/'.$template.'/images/monthdot_'.$event_calno.'.gif" alt="" width="4" height="9" border="0" />';
								...
								$start2		 = date('G:i', $val['start_unixtime']);
								if ($type == 'large') {
									$switch['EVENT'] .= '<div class="V9"><!--img src="templates/'.$template.'/images/monthdot_'.$event_calno.'.gif" alt="" width="4" height="9" border="0" /-->';
 									$switch['EVENT'] .= openevent($daylink, $cal_time, $uid, $val, $month_event_lines, 20, 'organizers-event', "$start2 - ").'<br />';
									$switch['EVENT'] .= '</div>';
								} else {
									// $switch['EVENT'] = '<img class="tooltip" title="There is an event" src="templates/'.$template.'/images/event_dot.gif" alt=" " width="11" height="10" border="0" />';
									$tooltip_text = '';
									$nb_events = 0;
									foreach ($master_array[$daylink] as $event_times_tooltip) {
										foreach ($event_times_tooltip as $val_tooltip) {
											$nb_events++;
											$tooltip_text.= substr($val_tooltip['event_start'],0,2) . ':' . substr($val_tooltip['event_start'],2,2) .' '.urldecode($val_tooltip['event_text']).'<br>';
										}
									}
									$switch['EVENT'] = '<a href="day.php?cal=' . $cal . '&getdate=' . $daylink . '" class="tooltipped" data-tooltip-content="#tooltip_' . $daylink . '">' . date ("j", $start_day) . '</a>';
									
									$switch['EVENT'].= '<div class="tooltip_templates"><span id="tooltip_' . $daylink . '">' . $tooltip_text  /*json_encode($event_times)*/ . '</span></div>';
								}
							}
						}
					}
			...
			if( !$switch['EVENT'] && $type == 'medium' ){
				$switch['EVENT'] = date ("j", $start_day);
			}
